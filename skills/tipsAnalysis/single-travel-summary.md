...
- 每个条目整合：总结 + 费用 + 时长 + 注意事项 + 经纬度
- 同一景点在不同维度中出现的 → 合并为一条
- 去重、删除空值、冲突取明确值

## 地理位置获取（重要）

每个景点/项目/美食/住宿必须附带**经纬度坐标**：

- 使用 `get_location` 工具获取：传入 `address`（场所名称）和 `city`（所在城市）
- 工具返回 `{"lng": 经度, "lat": 纬度}`
- **每个地点独立调用一次**，不要编造坐标
- 工具调用失败 → `coordinates` 填 `null`，不要编造
...
6. **地理位置**：每个 `spots[].coordinates` 必须包含经纬度，通过 `get_location` 工具获取；工具失败时填 `null`，不得编造
