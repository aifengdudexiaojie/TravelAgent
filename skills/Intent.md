...
- 支持相对日期表达（**基准 = 运行时注入的当前日期 `{{CURRENT_DATE}}`，不是文档里的示例日期**）：
  - "这周末" → 推算本周周六-周日
  - "下周" → 推算下周对应的日期范围
  - "明天"、"后天" → 对应具体日期
  - "8月" → 若当前是 7 月，则 start=当年 08-01, end=当年 08-31
...
> ⚠️ **示例中的日期只为演示格式，与真实"当前日期"无关，绝对不要照抄。**
> 真实当前日期见运行时注入的 `current_date`（即 `{{CURRENT_DATE}}`）。
> 所有相对日期（明天/下周/8月…）都必须以注入的当前日期重新推算。
...
1. **日期推算（最重要）**：**以运行时注入的「当前日期」为唯一基准**推算所有相对日期。
   当前日期会在对话消息里以 `'current_date':'YYYY-MM-DD'` 给出（同时 system prompt 里的
   `{{CURRENT_DATE}}` 已被替换为真实日期）。

   - **绝对禁止**使用本文档示例中的任何具体日期作为"今天"（示例只演示格式）。
   - 若消息里没有给出 `current_date`，则不要输出任何相对日期推算结果，
     `start_date` / `end_date` 一律为 `null`，宁可不猜也不要编造。
   - 推算示例（假设注入的当前日期是 `{{CURRENT_DATE}}`，星期 `{{WEEKDAY}}`）：
     - "明天" → 当前日期 +1 天；"后天" → +2 天；"大后天" → +3 天
     - "这周末" → 本周六 ~ 本周日（若今天已是周日，则取下一个周六日）
     - "下周" → 下周一到下周日；"下个月" → 下月 1 号起
     - "8月3日" → 若该日期已过，则取**明年** 8 月 3 日；否则取今年
     - "去年国庆" → 去年 10-01 ~ 10-07（过去行程，允许早于当前日期）
     - "明年春节" → 明年春节对应的公历日期
   - `days` 与 `start_date`/`end_date` 的关系：**同行天数按自然日计算**，
     即 `end_date = start_date + days - 1`（例如 3 天行程：10-01 ~ 10-03）。
...
9. **日期格式**：`start_date` / `end_date` 一律 `YYYY-MM-DD`（补零），不要带星期、不要用中文日期
