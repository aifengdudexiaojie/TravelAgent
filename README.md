...
- 扫码后**必须在手机上点「确认登录」**；二维码过期不会自动换新（重复取码会让刚才的扫码失效），点「🔄 重新获取二维码」即可
- ⚠️ 上游每次查登录状态都会**新起一个 Chromium**（实测 ~4 秒），所以发码后前端/后端进入"静默期"：不查 MCP，只看 `cookies.json` 与 MCP 日志判断结果；常态查询也缓存 30 秒（`XHS_STATUS_CACHE`）
- 若手机提示「安全验证 / 请再次扫码」（小红书风控的二次验证，上游 MCP 暂不支持）→ 改用「📄 导入 cookies.json」把本机登录态搬过来（弹窗里有诊断面板与指引）
