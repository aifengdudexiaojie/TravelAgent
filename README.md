...
- 点按钮 → **网页弹出二维码** → 用小红书 App 扫码 → 手机上点「确认登录」→ 灯变绿
- 二维码由**服务器自己的浏览器实时读取**（每 2.5 秒更新），所以不会扫到过期码；
  若小红书弹出「安全验证 / 请再次扫码」，页面会把**那张验证码**也显示出来，再扫一次即可
  （上游 MCP 只截一张静态码、也检测不到二次验证弹窗，见 issue #799 —— 这也是我们自己驱动登录浏览器的原因）
- 服务器**不需要**桌面环境/浏览器窗口，用户也**不需要**上传任何文件
- ⚠️ Linux 服务器请先执行一次登录环境安装（含浏览器运行库）：
  ```bash
  sudo bash deploy/install-xhs-deps.sh     # MCP 内置 Chromium 的系统依赖
  sudo bash deploy/install-xhs-login.sh    # playwright + Xvfb（虚拟显示）
  ```
- 一人一实例（独立工作目录 + 独立端口）：cookies 互不覆盖、账号互不干扰，空闲 30 分钟自动回收
- 换号：点「🔄 换个账号」→ 自动退出登录并出新码
